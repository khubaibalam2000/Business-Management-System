...
//   const [bid, setBid] = useState("");
  ...
  useEffect(() => {
    axios.get('/we/BatchesView.php').then(response => {
  ...
  const onSubmit = (data) => {
    ...
    axios.post("/we/BatchesInsert.php", data).then(res => console.log(res.data));
    axios.get('/we/BatchesView.php').then(response => {
      setRows({ rows2: response.data});
      setUser(rows2 ? Object.values(rows2)[0] : null);
      setRec_id();
  ...
  const onUpdate = () => {
    let item={name, bs, rs, rec_id};
    console.log("item", name, bs, rs, rec_id);
    axios.post('/we/BatchesUpdate.php', item).then(res => console.log(res.data));
    axios.get('/we/BatchesView.php').then(response => {
      setRows({ rows2: response.data});
      setUser(rows2 ? Object.values(rows2)[0] : null);
      setRec_id();
  ...
  const onDelete = (data) => {
      console.log(data);
    axios.get("/we/BatchesDelete.php", { params: { id: data } }).then(console.log("data")).catch(err => console.log(err));
    axios.get('/we/BatchesView.php').then(response => {
      setRows({ rows2: response.data});
      setUser(rows2 ? Object.values(rows2)[0] : null);
      setRec_id();
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="center">{row.bs}</TableCell>
        ...
        />
          ...
          <Modal2 ref={modalRef2}>
            ...
            <FormControl variant="standard" >
              ...
              <br></br>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <div><TextField {...register("bs", { required: true })} value={bs} onChange={(e)=>{setBs(e.target.value)}} style={{ width: "240%"}} id="standard-basic" label="Batch Status" variant="standard" /></div>
              </div>
              <br></br>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <div style={{width: "60%"}}><TextField {...register("rs", { required: true })} type="text" value={rs} onChange={(e)=>{setRs(e.target.value)}} id="standard-basic" label="Defect Status" variant="standard" /></div>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <div>
        ...
        >
          ...
          <Modal2 ref={modalRef}>
            ...
            <FormControl variant="standard" >
              ...
              <br></br>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <div><TextField {...register("bs", { required: true })} style={{ width: "240%"}} id="standard-basic" label="Batch Status" variant="standard" /></div>
              </div>
              <br></br>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <div style={{width: "60%"}}><TextField {...register("rs", { required: true })} type="text" id="standard-basic" label="Defect Status" variant="standard" /></div>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <div>
