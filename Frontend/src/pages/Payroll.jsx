...
//   const [rs, setRs] = useState("");
  ...
  const onSubmit = (data) => {
    ...
    axios.post("/we/PayrolInsert.php", data).then(res => console.log(res.data));
    axios.get('/we/EmpView.php').then(response => {
      setRows({ rows2: response.data});
      setUser(rows2 ? Object.values(rows2)[0] : null);
      setRec_id();
  ...
  const onUpdate = () => {
    let item={inc, bon, rec_id};
    console.log("item", inc, bon, rec_id);
    axios.post('/we/PayrolInsert.php', item).then(res => console.log(res.data));
    axios.get('/we/EmpView.php').then(response => {
      setRows({ rows2: response.data});
      setUser(rows2 ? Object.values(rows2)[0] : null);
      setRec_id();
      ...
      <Paper sx={{ width: '100%', mb: 2 }}>
        ...
        />
          ...
          <Modal2 ref={modalRef2}>
            ...
            <br></br>
            <form >
            <FormControl variant="standard" ><div style={{ display: "flex", justifyContent: "space-between" }}>
                <div style={{width: "100%"}}><TextField {...register("rec_id", { required: true })} value={rec_id} type="text"  id="standard-basic" label="Employee ID" variant="standard" /></div>
              </div>
              <br></br>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <div style={{width: "100%"}}><TextField {...register("inc", { required: true })} type="text" onChange={(e)=>{setInc(e.target.value)}} id="standard-basic" label="Increment amount" variant="standard" /></div>
              </div>
              <br></br>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <div><TextField {...register("bon", { required: true })} style={{ width: "240%"}} onChange={(e)=>{setBon(e.target.value)}} id="standard-basic" label="Bonus Amount" variant="standard" /></div>
              </div>
              <br></br>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <div>
                  <Fab style={{marginLeft: "330%"}} onClick={onUpdate} type="submit" variant="extended" size="medium" color="primary" aria-label="add">
                    Submit
                  </Fab>
                </div>
